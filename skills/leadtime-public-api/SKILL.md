...
Use this skill only when the user asks the agent to build, debug, or plan scripts, automations, Zapier/n8n-style connectors, or other external integrations with the Leadtime Public API.

For normal Leadtime workspace operations inside the agent, prefer the bundled Leadtime MCP server instead of telling the user to create a personal access token.
